...
class DFTests(unittest.TestCase):								                      	#defining functions for unit tests to conduct on code
    def setUp(self, df1, df2):
        try:													                        #try used to catch and handle exceptions during unit tests after matching data i.e. Operator and price lists
            ...
            expected_df1 = pd.DataFrame({'Operator': [1, 268, 46, 4620, 468, 4631, 4673, 46732], 'Cost_OperatorA': [0.9, 5.1, 0.17, 0.0, 0.15, 0.15, 0.9, 1.1]})	#expected_df1 defines data which is to be matched with data in body of code
            expected_df2 = pd.DataFrame({'Operator': [1, 44, 46, 467, 48], 'Cost_OperatorB': [0.92, 0.5, 0.2, 1.3, 1.2]})
...
if __name__ == '__main__':
    ...
    for a in range(0, m + 1):  									              	        #iteration run to find match of prefix in input
            ...
            df1 = pd.DataFrame({'Operator': [1, 268, 46, 4620, 468, 4631, 4673, 46732],	'Cost_OperatorA': [0.9, 5.1, 0.17, 0.0, 0.15, 0.15, 0.9, 1.1]})	    #assigned dataframes as Operators and Price Lists in pandas package
            df2 = pd.DataFrame({'Operator': [1, 44, 46, 467, 48], 'Cost_OperatorB': [0.92, 0.5, 0.2, 1.3, 1.2]})
            DFTests1.setUp(df1, df2)													#calling DFTests function to conduct test with expected and my dataframes
            df = pd.merge(df1, df2, how='outer', on='Operator',) 						#merging current dataframes together in outer way so all values are merged
            df['Minimum_Price'] = df[['Cost_OperatorA', 'Cost_OperatorB']].min(axis=1)	#calculating Minimum price in dataframes, and axis=1 for rows which are the costs in Operators
            if not df.loc[df['Operator'] == n].empty:  				                    #if not statement to assign minimum price in both operators
                f = df.loc[df['Operator'] == n]				    		                #assigning f as dataframe matching prefix with costs
                g = df.loc[df['Operator'] == n, 'Minimum_Price']   	                    #assign g with value of Minimum price
